...
class UserController {
            ...
            .then((user) => {
                if (!user) {
                    ...
                    if (!checkPassword(req.body.password, user.password)) {
                        ...
                        sendEmail(user.dataValues.email, val)
                        realVal[user.dataValues.email] = { email: user.dataValues.email, payload: val }
                        const token = generateToken(user)
                        res.status(200).json({
                            status: 'Success',
    ...
    static validation(req, res, next) {
        if (!realVal[req.user.email]) {
            next({
                status: 404,
                message: 'Invalid token, try OTP code again'
            })
            return
        }
